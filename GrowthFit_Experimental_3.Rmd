...
* This model differs from the one depicted in `GrowthFit_Experimental_3.Rmd` in that it incorporates relative weight as a predictor of costs nto growth.
...
$$
M_{j,t} = a_3 e^{c_2 r_{j,t}} \bigg (\frac{w_{j,t}}{\bar{w}} \bigg )^{b_3} e^{\eta_j}.
$$
Here, $\delta_j$ and $\eta_j$ describe individual-specific deviations from the population median intake and cost, respectively, and we assume they come from a bivariate normal distribution with correlation $\rho$.
The parameter $c_2$ allows relative weight to impact the cost, either postively or negatively.
...
for (jj in 1:J) {
  ...
  num_ws <- length(ws)  # weight samples of jj
  if (!is.null(num_ws)) {
  	for (kk in 1:num_ws) {
      m_d[jj,kk] <- ds[kk]
      m_w[jj,kk] <- ws[kk]
...
```{r}
# fit the model!
fit_n <- stan(file = 'IntakeFit_Experimental_3.stan', data = stan_dat,
  iter = 350, warmup = 150, chains = 1, refresh = 15, seed = 1942)
```

```{r}
# check out some predicted model parameters
model_par <- c("a1", "b1", "c1", "a2", "b2", "rho",
	"a3", "b3", "c2", "a4", "eta", "sigma_k")
...
c2   <- rstan::extract(fit_n, pars = "c2")[[1]]
