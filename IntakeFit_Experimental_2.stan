...
parameters {
  ...
  real<lower= -3.0, upper= 1.0>  ln_a2;     // log-cov (at standard weigth)
  ...
  real<lower= -0.5, upper= 0.5>  ln_b3;     // cost-weight power
...
model {
  ...
  c1    ~ normal( 0.00, 0.50); //  
  ln_a2 ~ normal(-1.00, 1.00); //  
  b2    ~ normal( 0.00, 0.50); //  
  // growth parameters
  ln_eta ~ normal( 0.3, 0.4); //  
  ln_a3  ~ normal( 1.5, 0.2); //  
  ln_b3  ~ normal( 0.0, 0.2); //  
  ...
  for (i in 1:I) { // for each obxervation
        ...
        for (dd in (m_d[jj,ss]):(m_d[jj,ss+1]-1)) { // update each day
          wP[dd+1] = wP[dd] + 
            eta*a1*exp(c1*m_r[jj,dd] + u[1,jj])*pow(wP[dd]/w_std, b1) -
                a3*exp(                u[2,jj])*pow(wP[dd]/w_std, b3);
        }
