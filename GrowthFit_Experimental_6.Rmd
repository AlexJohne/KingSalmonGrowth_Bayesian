...
In this document we build on the intake model to incorporate costs to
resolve fish growth. Fish intake depends on a fish's weight and its
weight relative to others in the same tank.

-   This model differs from the one depicted in
    `GrowthFit_Experimental_4.Rmd` in that it incorporates relative
    weight as a predictor of costs nto growth.
...
Here, we assume energy density is constant, which means rates of change
in fish weight can be expressed in terms of weight-dependent gains and
losses. Let $w_{j,t}$ denote the weight of fish $j$ on day $t$, and
$r_{j,t}$ be its relative weight compared with the average weight of
others in the tank (\$). The expected intake rate of this individual on
day $t$ is

$$
I_{j,t} = a_1 e^{(c_{1,0} + c_{1,1}\bar{w}) r_{j,t}} \bigg (\frac{w_{j,t}}{\hat{w}} \bigg )^{b_1} e^{\delta_j}.
$$

Variation in intake is assumed to be consistent with a gamma
distribution with constant coefficient of variation: $$
\text{cv}_{j,t} = a_2 .
$$\
Note: $$
\alpha = \text{cv}_{j,t}^{-2}
$$ and $$
\beta = \alpha/I_{j,t}
$$.

The daily gain in weight of this fish is $\varepsilon I_{j,t}$ and the
daily cost in weight is

$$
C_{j,t} = a_2 e^{(c_{2,0} + c_{2,1}\bar{w}) r_{j,t}} \bigg (\frac{w_{j,t}}{\hat{w}} \bigg )^{b_2} e^{\eta_j}
$$

Here, $\delta_j$ and $\eta_j$ describe individual-specific deviations
from the population median intake and cost, respectively, and we assume
they come from a bivariate normal distribution with correlation $\rho$.
The parameter $c_2$ allows relative weight to impact the cost, either
...
Weights are provided at four times: $t = \{T_0, T_1, T_2, T_3 \}$. To
implement the growth model requires daily estimates of relative size and
mean weights, which can both be estimated using linear regression:
...
$$
\bar{w}_{j,t} = \bar{w}_{j,T_i} + \frac{t - T_i}{T_{i+1} - T_i} \left ( \bar{w}_{j,T_{i+1}} - \bar{w}_{j,T_i} \right ),
$$

These mean weights and relative weights are calculated prior to fitting
and supplied to the stan fitting function.

Note that when applying this bio-energetic model it is common to assume
$b_1 = 2/3$ and $b_2 = 1$ (or even less; $b_2 = 0.8$) (Essington et al.,
2001). An objective of this analysis is to see if these assumptions are
consistent with our intake and growth data.

## Correlation between relative intake and relative cost

The stan model `IntakeFit_Experimental_2.stan` estimates the correlation
between the individual-level random effects associated with intake and
costs, but fitting this correlation is a little complicated. First, for
each individual fish, uncorrelated standard normals are generated for
intake and costs, respectively. These paired random effects are then
correlated by a linear transformation. The stan code
`diag_pre_multiply(sigma_u, L_u)` fits a lower diagonal matrix: $$
L = \begin{pmatrix} a & 0\\ b & c \end{pmatrix}
$$ where $a^2$ is the variance of the residuals associated with intake.
The covariance between the intake and cost residuals is $$
\text{Cov}(aZ_\text{I},bZ_\text{I}+cZ_\text{C}) = ab
$$ where $Z_\text{I}$ and $Z_\text{C}$ are the uncorrelated standard
normals for intake and costs, respectively. The variance of the
residuals associated with costs is $(b^2 + c^2)$. The correlation
between the two random effects $X_\text{I} = aZ_\text{I}$ and
$X_\text{C} = bZ_\text{I} + cZ_\text{C}$ is $$
\rho = \frac{\text{Cov}(X_\text{I},X_\text{C})} {\sqrt{ \text{Var}(X_\text{I}) \text{Var}(X_\text{C}) }} = \frac{ab}{\sqrt{a^2(b^2+c^2)}}
$$ This last equation is implemented in `generated quantities` to
estimate $\rho$.
...
#df_indiv_gsi <- readRDS("df_indiv_GSI.rds")
...
There are `r nrow(df_indiv)` observations spanning
`r length(levels(df_indiv$ID))` individuals.
...
#     # net daily gain (no eta)
#   df_predict$NG_025[i] <- quantile(
#     a1*(df_predict$w[i]/w_std)^b1 - a2*(df_predict$w[i]/w_std)^b2, probs = 0.025)
#   df_predict$NG_500[i] <- quantile(
#     a1*(df_predict$w[i]/w_std)^b1 - a2*(df_predict$w[i]/w_std)^b2, probs = 0.500)
#   df_predict$NG_975[i] <- quantile(
#     a1*(df_predict$w[i]/w_std)^b1 - a2*(df_predict$w[i]/w_std)^b2, probs = 0.975)
}
...
# for (jj in 1:J) { # for each fish (no eta --> absorbed by a1??)
#   w <- array(data = 0.0, dim = c(D, Reps)) # all posterior predictions for fish jj
#   if (v_w[jj] > 1) { # more than 1 weight so growth to predict
#     for (ss in 1:(v_w[jj] - 1)) { # for each growth interval
#       # predict weight change for individual jj from sample time ss -> ss+1
...
#       for (dd in (m_d[jj,ss]):(m_d[jj,ss+1]-1)) { # update weight each day
#         w[dd+1, ] <- w[dd, ] + 
#           a1*exp((c10 + c11*m_mu[jj,dd])*m_r[jj,dd] + RE_I_j[ ,jj])*((w[dd, ]/w_std)^b1) -
#               a2*exp((c20 + c21*m_mu[jj,dd])*m_r[jj,dd] + RE_C_j[ ,jj])*((w[dd, ]/w_std)^b2) 
#       }
#       EwP[jj, ]   <- apply(w, 1, mean) # calculate mean weight for each day
#       wP[jj,ss+1] <- mean(w[dd+1, ])   # store mean of predicted weights for sample day
...
```{r comment=''}
cat(readLines('IntakeFit_Experimental_6_2.stan'), sep = '\n')
```
